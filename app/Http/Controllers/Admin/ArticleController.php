...
use App\Http\Models\ArticlePhoto;
			...
			if(count($input['images']) > 0)
			{
				foreach($input['images'] as $img)
				{
					if(isset($img) && is_uploaded_file($img))
					{
						...
						$dir = public_path('images/article/'.$article->id.'/photos');
						if(!File::isDirectory($dir)) {
							File::makeDirectory($dir, 0777, true);
						}
						...
						$dir_small = public_path('images/article/'.$article->id.'/small');
						if(!File::isDirectory($dir_small)) {
							File::makeDirectory($dir_small, 0777, true);
						}
						$image = Image::make($img)
							->fit(100, 100)
							->save($dir_small.'/'.$name.'.jpg');
			...
			if($article = Article::find($id))
			{
				...
				if(isset($input['remove_photos']) && !empty($input['remove_photos']))
				{
					...
					foreach($remove_photos as $photo_id) {
						$photo = ArticlePhoto::find($photo_id);
						$photo->delete();
					}
				}
